...
class TunggakanDetail: UIViewController {
    ...
    func readBulan() {
        ...
        db.collection("transaksi").document(TunggakanController.temp_id).collection("bulan").addSnapshotListener { (querySnapshot, err) in
                    ...
                    if transaksi.isEmpty {
                        ...
                        DispatchQueue.main.async(execute : {
                            ...
                            if self.bulanList[self.bulanList.count-1].status == "lunas" {
                                ...
                                self.db.collection("transaksi").document(TunggakanController.temp_id).collection("bulan").addSnapshotListener { (querySnapshot, err) in
                                    if let err = err {
                                        print("Error getting documents: \(err)")
                                    } else {
                                        if let nasabah = querySnapshot?.documents {
                                            if nasabah.isEmpty {
                                                ...
                                                for i in 0..<self.bulanListDelete.count {
                                                    ...
                                                    self.db.collection("transaksi").document(TunggakanController.temp_id).collection("bulan").document(a!).delete { err in
                                                        if let err = err {
                                ...
                                self.db.collection("transaksi").document(TunggakanController.temp_id).delete { err in
                                    if let err = err {
                                        print("Error removing document: \(err)")
                                    } else {
                                        print("Document successfully removed!")
                ...
                if let transaksi = querySnapshot?.documents {
                    if transaksi.isEmpty {
                        ...
                        for i in 0..<self.transaksiList.count {
                            if TunggakanController.temp_id == self.transaksiList[i].transaksi_id  {
                                ...
                                self.temp_index = self.transaksiList[i].index_transaksi!
...
extension TunggakanDetail: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if bulanList[indexPath.row].status == "lunas" {
        ...
        } else if temp_index == bulanList[indexPath.row].index {
            ...
            let update = UIAlertAction(title: "Ya", style: .destructive) { (alertAction) in
                ...
                let close = UIAlertAction(title: "Tutup", style: .cancel) { (alertAction) in
                    ...
                    let tunggak = ["status_cicilan": "aman",
                                   "kondite": "buruk"]
                    self.db.collection("transaksi").document(TunggakanController.temp_id).updateData(tunggak)
